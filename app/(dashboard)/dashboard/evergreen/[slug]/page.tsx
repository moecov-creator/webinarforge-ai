    ...
    <div className="flex-1 flex overflow-hidden">
      <div className="flex-1 flex flex-col">
        ...
        <div className="px-6 py-4 border-t border-white/5 bg-black/20 flex-shrink-0">
          <div className="h-1 bg-white/10 rounded-full mb-3 cursor-pointer" onClick={(e) => { const rect = e.currentTarget.getBoundingClientRect(); setCurrentTime(Math.round(((e.clientX - rect.left) / rect.width) * duration)); }}>
      ...
      <div className="w-72 border-l border-white/5 flex flex-col bg-black/20 flex-shrink-0">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {visibleComments.length === 0 && (
            <div className="text-center pt-8"><p className="text-xs text-white/20">Comments will appear as the webinar plays</p></div>
          )}
...
function SettingsTab() {
  ...
  const [offers, setOffers] = useState<{ id: string; name: string; showAt: number }[]>([]);
  ...
  const sections = [
    ...
    { key: "offers", label: "Offers", icon: Gift, badge: offers.length || undefined },
    { key: "handouts", label: "Handouts", icon: FileText },
    { key: "polls", label: "Polls", icon: BarChart },
    { key: "chat", label: "Chat", icon: MessageSquare },
    { key: "chatsim", label: "Chat Simulator", icon: MessageSquare, badge: chatMessages.length || undefined },
  ...
  const generateChatMessages = () => {
    ...
    setTimeout(() => {
      const names = ["Sarah M.", "Marcus T.", "Jennifer K.", "David R.", "Lisa P.", "Kevin W.", "Amanda S.", "Brian L."];
      const messages = [
        "This is exactly what I needed! 🙌", "Taking notes on everything here!",
        "Wow, this changes how I think about this", "Can you say more about that?",
        "This is gold. Pure gold.", "Already seeing how I can apply this",
        "Does this work for beginners too?", "The results speak for themselves 🔥",
        "Sharing this with my whole team", "This is why I signed up!",
      ];
      const generated = Array.from({ length: 12 }, (_, i) => ({
        id: String(i),
        name: names[i % names.length],
        message: messages[i % messages.length],
        showAt: (i + 1) * 120 + Math.floor(Math.random() * 60),
      }));
  ...
  return (
    <div className="flex-1 flex overflow-hidden">
      {/* Sidebar */}
      <div className="w-52 border-r border-white/5 bg-black/10 flex flex-col py-3 shrink-0">
        {sections.map(s => (
          <button
            key={s.key}
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {activeSection === "general" && (
          <div className="max-w-lg space-y-5">
              ...
              <div className="flex gap-2">
                {[["Hours", "1"], ["Minutes", "30"], ["Seconds", "0"]].map(([label, val]) => (
                  <div key={label} className="flex-1">
                    ...
                    <input defaultValue={val} type="number" className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-purple-500" />
                  </div>
                ))}
              </div>
            </div>
            ...
            {redirectAfter && (
              <input placeholder="Redirect URL" className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-purple-500" />
            )}
            <div className="flex items-center justify-between py-3">
              <span className="text-sm text-white/70">Waiting image</span>
              <button className="px-3 py-1.5 bg-purple-600 text-white text-xs font-semibold rounded-lg">Edit</button>
            </div>
          </div>
        )}
        ...
        {activeSection === "video" && (
          <div className="max-w-lg space-y-5">
            ...
            <div className="border-2 border-dashed border-white/10 rounded-xl p-8 text-center hover:border-white/20 transition-colors cursor-pointer">
              ...
              <p className="text-sm text-white/40">Drop video file here or click to browse</p>
              <p className="text-xs text-white/20 mt-1">MP4, MOV up to 2GB</p>
              <button className="mt-3 px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-xs text-white/50 hover:bg-white/10 transition-colors">Choose File</button>
            </div>
        ...
        {activeSection === "offers" && (
          <div className="max-w-lg space-y-5">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => setOffers(prev => [...prev, { id: Date.now().toString(), name: "New Offer", showAt: 2700 }])}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-purple-600 text-white text-xs font-semibold rounded-lg"
              >
              ...
              offers.map(o => (
                <div key={o.id} className="flex items-center justify-between p-4 bg-white/3 border border-white/8 rounded-xl">
                  <div>
                    ...
                    <p className="text-xs text-white/30 mt-0.5">Shows at {Math.floor(o.showAt / 60)}:{String(o.showAt % 60).padStart(2,"0")}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <button className="px-2 py-1 border border-white/10 text-white/40 text-xs rounded-lg hover:border-white/20">Edit</button>
                    <button onClick={() => setOffers(p => p.filter(x => x.id !== o.id))} className="text-red-400/60 hover:text-red-400 text-lg leading-none">×</button>
                  </div>
                </div>
              ))
            )}
        ...
        {activeSection === "handouts" && (
          <div className="max-w-lg space-y-5">
            <div className="flex items-center justify-between">
              ...
              <button className="flex items-center gap-1.5 px-3 py-1.5 bg-purple-600 text-white text-xs font-semibold rounded-lg">
                <Plus className="w-3 h-3" /> Add handout
              </button>
            </div>
            <div className="text-center py-12 border border-dashed border-white/10 rounded-xl">
              <FileText className="w-8 h-8 text-white/10 mx-auto mb-2" />
              <p className="text-sm text-white/30">No handouts yet</p>
              <p className="text-xs text-white/20 mt-1">Add PDFs or resources for attendees to download</p>
            </div>
          </div>
        )}
        ...
        {activeSection === "polls" && (
          <div className="max-w-lg space-y-5">
            ...
            <div className="text-center py-12 border border-dashed border-white/10 rounded-xl">
              ...
              <p className="text-sm text-white/30">No polls yet</p>
              <p className="text-xs text-white/20 mt-1">Engage attendees with timed poll questions</p>
            </div>
          </div>
        )}
          ...
          <div className="max-w-lg space-y-5">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                {chatMessages.length > 0 && (
                  <button onClick={() => setChatMessages([])} className="text-xs text-red-400/60 hover:text-red-400">Remove all</button>
                )}
                <button
            ...
            {chatMessages.length === 0 ? (
              <div className="text-center py-12 border border-dashed border-white/10 rounded-xl">
                ...
                <p className="text-xs text-white/20 mb-4">Generate AI chat messages based on your webinar script — they'll appear at key moments during the presentation.</p>
                <button
                  onClick={generateChatMessages}
                  disabled={generatingChat}
              ...
              <div className="space-y-2">
                {chatMessages.map((m, i) => (
                  <div key={m.id} className="flex items-center justify-between p-3 bg-white/3 border border-white/8 rounded-xl hover:border-white/15 transition-colors">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-medium text-white/70 truncate">{m.name} · <span className="text-white/40">{m.message}</span></p>
                        <p className="text-[10px] text-white/25 mt-0.5">Shows at {Math.floor(m.showAt / 60)}:{String(m.showAt % 60).padStart(2,"0")}</p>
                      </div>
                    </div>
                    <button onClick={() => setChatMessages(p => p.filter((_, j) => j !== i))} className="text-white/20 hover:text-red-400 text-lg leading-none ml-2">×</button>
                  </div>
                ))}
              </div>
            )}
        ...
        {activeSection === "colors" && (
          <div className="max-w-lg space-y-5">
            ...
            {[["Primary color", "#7C3AED"], ["Button color", "#7C3AED"], ["Background color", "#06060F"], ["Text color", "#FFFFFF"]].map(([label, def]) => (
              <div key={label} className="flex items-center justify-between py-3 border-b border-white/5">
                <span className="text-sm text-white/70">{label}</span>
                <div className="flex items-center gap-2">
                  <div className="w-6 h-6 rounded border border-white/10" style={{ background: def }} />
        ...
        {activeSection === "labels" && (
          <div className="max-w-lg space-y-5">
            ...
            {[["Register button text", "REGISTER NOW"], ["Viewer count label", "watching"], ["Chat input placeholder", "Type your message..."], ["Live badge text", "LIVE"]].map(([label, def]) => (
              <div key={label}>
        ...
        {activeSection === "embed" && (
          <div className="max-w-lg space-y-5">
            ...
            <div>
              ...
              <textarea readOnly className="w-full h-16 bg-white/3 border border-white/10 rounded-xl p-3 text-xs font-mono text-white/50 resize-none focus:outline-none"
                value={`<link rel="stylesheet" href="https://webinarforge.ai/css/room.css">`} />
            </div>
            <button className="px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white/50 hover:bg-white/8 transition-colors">
              Copy embed code
            </button>
          </div>
        )}
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="rounded-xl bg-white/3 border border-white/8 overflow-hidden">
          <div className="px-6 py-5 border-b border-white/5">
            ...
            <div className="flex items-center justify-center gap-4 mt-3">
              <span className={`text-xs ${regMode === "platform" ? "text-white" : "text-white/30"}`}>Build on our platform</span>
              <Toggle value={regMode === "embed"} onChange={v => setRegMode(v ? "embed" : "platform")} />
              <span className={`text-xs ${regMode === "embed" ? "text-white" : "text-white/30"}`}>Embed on your site</span>
            </div>
          </div>
          {regMode === "embed" ? (
            <div className="p-8 flex flex-col items-center justify-center min-h-40 cursor-pointer" style={{ background: "linear-gradient(135deg, #4F6EF7, #3451D1)" }}>
              ...
              <div className="mt-4 bg-white/10 rounded-lg px-5 py-2 text-white text-sm font-medium">Open embed builder →</div>
            </div>
          ) : (
            <div className="p-8 flex flex-col items-center justify-center min-h-40" style={{ background: "linear-gradient(135deg, #059669, #047857)" }}>
              ...
              <button className="mt-4 bg-white text-green-700 font-semibold text-sm px-5 py-2 rounded-lg">Open page builder →</button>
            </div>
          )}
        </div>
        ...
        <div className="rounded-xl bg-white/3 border border-white/8 overflow-hidden">
          <div className="px-6 py-5 border-b border-white/5">
            ...
            <div className="flex items-center justify-center gap-4 mt-3">
              <span className={`text-xs ${tyMode === "platform" ? "text-white" : "text-white/30"}`}>Build on our platform</span>
              <Toggle value={tyMode === "embed"} onChange={v => setTyMode(v ? "embed" : "platform")} />
              <span className={`text-xs ${tyMode === "embed" ? "text-white" : "text-white/30"}`}>Embed on your site</span>
            </div>
          </div>
          <div className="p-8 flex flex-col items-center justify-center min-h-40" style={{ background: "linear-gradient(135deg, #059669, #047857)" }}>
...
export default function EvergreenRoomPage() {
  ...
  return (
    <div className="h-screen bg-[#06060f] flex flex-col overflow-hidden">
      ...
      {/* Tab bar */}
      <div className="flex items-center gap-0 px-6 border-b border-white/5 bg-black/10 flex-shrink-0">
        {TABS.map(tab => (
          <button
            key={tab.key}
        ...
        <div className="ml-auto flex items-center gap-2 py-2">
          <button className="px-3 py-1.5 bg-purple-600 hover:bg-purple-700 text-white text-xs font-semibold rounded-lg transition-colors">
            Save changes
          </button>
          <a href="#" target="_blank" className="flex items-center gap-1 px-3 py-1.5 border border-white/10 hover:border-white/20 text-white/40 hover:text-white text-xs font-medium rounded-lg transition-colors">
