...
// ── Fallback chat generator (no video) ────────────────────────────
const FIRST_NAMES = ["Jason","Michael","David","James","Robert","Sarah","Jennifer","Amanda","Jessica","Ashley","Emily","Carlos","Miguel","Marcus","Priya","Tyler","Brandon","Patricia","Trevor","Monica","Tiffany","Jasmine","Keisha","Ethan","Logan"];
...
const CUE_RESPONSES: Record<CueType,(city:string)=>string[]> = {
  type1:       (city) => ["1","1️⃣","1 ✅","1 👍","1!",`1 from ${city}`,"1 🙌","1 absolutely","YES 1","Definitely 1","typing 1 rn","1 — mind blown","1 💯","1 no doubt","1 👊"],
  dropCity:    (city) => [city,`${city} 👋`,`Joining from ${city}!`,`Hello from ${city}`,`${city} in the house!`,`Watching from ${city} 🙌`,`Live from ${city}`,`${city} here!`,`${city} checking in!`],
  react:       (_)    => ["🔥🔥🔥","This is insane!","Mind blown 🤯","Taking notes!","WOW","🙌🙌🙌","Gold right here 💰","YESSS","👏👏👏","Screenshotting this!","💯💯💯","🚀🚀","Pausing to take notes 📝"],
  question:    (_)    => ["Does this work for beginners?","How long does this take?","Can I use this for B2B?","What software do you recommend?","How many hours per week?","What's the investment?","Is there a community?","How soon can I see results?","Do you offer coaching?"],
  testimonial: (_)    => ["I tried this last month and got 3 new clients! 🎉","This method got me to $10k/month in 60 days","Applied this and closed a $5k deal same week","$22k in revenue last month using this exact system","Finally broke 6 figures following these steps 🙏"],
  general:     (city) => ["So glad I showed up today!","Taking tons of notes 📓","Sharing this with my team",`Watching from ${city} — loving every minute`,"Pure value 💎","I needed to hear this today 🙏","This is the most practical advice I've heard all year"],
  joining:     (city) => [`Just joined from ${city}!`,`Hello from ${city} 👋`,`${city} just arrived!`,`Made it! Joining from ${city}`,`${city} checking in 🙋`,`Hey everyone! ${city} here`],
};
...
function generateFallbackChats(durationSeconds: number): SimChat[] {
  const chats: SimChat[] = [];
  let id = 0;
  CUE_TEMPLATES.forEach(cue => {
    ...
    for (let i=0;i<cue.burst;i++) {
      ...
      const showAt = Math.round(Math.min(baseTime+jitter, durationSeconds-10));
      chats.push({ id:String(id++), name:randomName(seed), city, message:responses[(seed+i)%responses.length], showAt, showAtFrac:showAt/durationSeconds, cueType:cue.type });
    }
  });
...
function rescaleSimChats(chats: SimChat[], newDuration: number): SimChat[] {
  return chats.map(c=>({...c, showAt:Math.round(c.showAtFrac*newDuration)})).sort((a,b)=>a.showAt-b.showAt);
}
...
function loadLS<T>(key:string,fb:T):T{try{const r=localStorage.getItem(key);return r?JSON.parse(r):fb;}catch{return fb;}}
...
function WatchRoomTab({videoSource,simChats,videoDuration,onVideoDuration}:{videoSource:VideoSource|null;simChats:SimChat[];videoDuration:number;onVideoDuration:(d:number)=>void}){
  ...
  const resolvedComments=MOCK_TIMED_COMMENTS.map(c=>({...c,triggerAt:Math.round(c.triggerAt*dur)}));
  const resolvedCTAs=MOCK_CTA_SEQUENCES.map(c=>({...c,triggerAt:Math.round(c.triggerAt*dur)}));
        ...
        <div className="px-6 py-4 border-t border-white/5 bg-black/20 flex-shrink-0">
          {!videoSource?(
            ...
            <div className="flex items-center gap-2 text-xs text-white/30">
              ...
              <span>Video synced · {formatTime(currentTime)} elapsed{videoDuration>0?` / ${formatTime(videoDuration)}`:""}{simChats.length>0?` · ${simChats.length} AI chat messages loaded`:""}</span>
            </div>
          )}
        </div>
      </div>
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2">
          {allMessages.length===0&&(
            <div className="text-center pt-8 px-4 space-y-2">
              {simChats.length>0?(<p className="text-xs text-white/20 leading-relaxed">▶ Press play to see {simChats.length} AI-generated messages fire in sync</p>):(<><p className="text-xs text-white/20">Generate chat in</p><p className="text-xs text-purple-400/60">Settings → Chat Simulator</p><p className="text-xs text-white/15">then press play</p></>)}
            </div>
          )}
          {allMessages.map(msg=>{
...
function ChatSimulatorSection({slug,simChats,onSimChatsChange,videoDuration,onVideoDuration}:{slug:string;simChats:SimChat[];onSimChatsChange:(c:SimChat[])=>void;videoDuration:number;onVideoDuration:(d:number)=>void}){
  ...
  const handleTranscribeUpload=(e:React.ChangeEvent<HTMLInputElement>)=>{
    const file=e.target.files?.[0];
    if(file) setTranscribeFile(file);
  };
  ...
  const handleTranscribe=async()=>{
    ...
    try {
      ...
      setTranscribeProgress("Generating contextual chat messages...");
  ...
  const handleFallbackGenerate=()=>{
    ...
    setTimeout(()=>{
      const chats=generateFallbackChats(dur);
      onSimChatsChange(chats);
      saveLS(`wf-chats-${slug}`,chats);
      if(!videoDuration&&manualDuration){const d=parseInt(manualDuration)*60;onVideoDuration(d);saveLS(`wf-duration-${slug}`,d);}
  ...
  const handleManualDuration=(val:string)=>{
    ...
    if(mins>0){
      ...
      if(simChats.length>0){const r=rescaleSimChats(simChats,d);onSimChatsChange(r);saveLS(`wf-chats-${slug}`,r);}
    }
  };
  ...
  return (
    <div className="max-w-2xl space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-white/30 mt-0.5">
            {simChats.length>0?`${simChats.length} messages · ${videoDuration>0?`synced to ${formatTime(videoDuration)} video`:"set video duration to sync"}`:"AI reads your video and generates contextual chat"}
          </p>
        </div>
        {simChats.length>0&&(
          <button onClick={()=>{onSimChatsChange([]);saveLS(`wf-chats-${slug}`,[]);}} className="text-xs text-red-400/60 hover:text-red-400 transition-colors">Remove all</button>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <button onClick={()=>setMode("ai")} className={`p-4 rounded-xl border-2 text-left transition-all ${mode==="ai"?"border-purple-500 bg-purple-500/10":"border-white/10 hover:border-white/20 bg-white/3"}`}>
          <div className="flex items-center gap-2 mb-1">
            <Sparkles className={`w-4 h-4 ${mode==="ai"?"text-purple-400":"text-white/30"}`}/>
            <span className={`text-sm font-semibold ${mode==="ai"?"text-white":"text-white/50"}`}>AI Video Analysis</span>
            <span className="text-[10px] px-1.5 py-0.5 bg-purple-500/20 text-purple-400 rounded-full">Recommended</span>
          </div>
        ...
        <button onClick={()=>setMode("fallback")} className={`p-4 rounded-xl border-2 text-left transition-all ${mode==="fallback"?"border-purple-500 bg-purple-500/10":"border-white/10 hover:border-white/20 bg-white/3"}`}>
          <div className="flex items-center gap-2 mb-1">
            <MessageSquare className={`w-4 h-4 ${mode==="fallback"?"text-purple-400":"text-white/30"}`}/>
            <span className={`text-sm font-semibold ${mode==="fallback"?"text-white":"text-white/50"}`}>Quick Generate</span>
          </div>
        ...
        <div className="space-y-4">
          <div className="p-4 bg-purple-500/8 border border-purple-500/20 rounded-xl">
            <div className="flex items-start gap-2 mb-2">
              ...
              <p className="text-xs text-purple-300 font-medium">How it works</p>
            </div>
            <ol className="text-xs text-white/35 space-y-1 ml-5 list-decimal leading-relaxed">
              <li>Upload your webinar video (MP4, MOV, WebM)</li>
              <li>OpenAI Whisper transcribes the full audio with timestamps</li>
              <li>GPT-4o analyzes the transcript and identifies key moments (value bombs, "type a 1" cues, Q&A, etc.)</li>
              <li>Generates 160 contextual comments that match what the presenter actually says</li>
            </ol>
          </div>
          ...
          {transcribing?(
            <div className="p-6 border border-purple-500/20 rounded-xl bg-purple-500/5 text-center space-y-3">
              <div className="flex items-center justify-center gap-2">
                <svg className="animate-spin h-5 w-5 text-purple-400" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"/></svg>
                <span className="text-sm text-purple-300 font-medium">Processing...</span>
              </div>
              <p className="text-xs text-white/40">{transcribeProgress}</p>
              <p className="text-[10px] text-white/20">Large videos may take 2–5 minutes. Don't close this tab.</p>
            </div>
          ):(
            <>
              <div>
                <label htmlFor="transcribe-video-input"
                  className={`flex flex-col items-center justify-center border-2 border-dashed rounded-xl p-8 text-center cursor-pointer group transition-all ${transcribeFile?"border-purple-500/50 bg-purple-500/5":"border-white/10 hover:border-purple-500/40 hover:bg-purple-500/5"}`}>
                  <Upload className={`w-8 h-8 mb-2 transition-colors ${transcribeFile?"text-purple-400":"text-white/20 group-hover:text-purple-400"}`}/>
                  {transcribeFile?(
                    <><p className="text-sm text-green-400 font-medium">{transcribeFile.name}</p><p className="text-xs text-white/30 mt-1">{(transcribeFile.size/1024/1024).toFixed(1)} MB · Click to change</p></>
                  ):(
                    <><p className="text-sm text-white/40 group-hover:text-white/60">Drop your webinar video here or <span className="text-purple-400 underline">browse</span></p><p className="text-xs text-white/20 mt-1">MP4, MOV, WebM, M4A · Up to 500MB</p><p className="text-xs text-white/15 mt-0.5">Audio-only files also work (M4A, MP3)</p></>
                  )}
                </label>
                <input id="transcribe-video-input" type="file" accept="video/*,audio/*,.mp4,.mov,.webm,.m4a,.mp3" className="hidden" onChange={handleTranscribeUpload}/>
              </div>
              ...
              >
                ...
                {transcribeFile?"Analyze Video & Generate Smart Chat →":"Upload a video first"}
              </button>
            </>
          )}
        </div>
      ...
      {mode==="fallback"&&(
        <div className="space-y-4">
          <div>
            <label className="text-xs text-white/40 mb-1 block">Video duration (minutes) — for proper chat spacing</label>
            <div className="flex gap-2">
              <input value={manualDuration} onChange={e=>handleManualDuration(e.target.value)} type="number" min="1" placeholder="e.g. 106 for 1h46m"
              ...
              <div className="flex items-center px-3 py-2 bg-white/3 border border-white/8 rounded-lg min-w-[80px] justify-center">
                <span className="text-xs text-white/30">{manualDuration?formatTime(parseInt(manualDuration||"0")*60):"hh:mm:ss"}</span>
              </div>
            </div>
          </div>
          <button onClick={handleFallbackGenerate} disabled={generatingFallback}
            className="w-full py-3 bg-white/8 hover:bg-white/12 border border-white/10 hover:border-white/20 disabled:opacity-50 text-white font-semibold rounded-xl transition-colors flex items-center justify-center gap-2">
          ...
          <p className="text-xs text-white/20 text-center">Messages will be spread proportionally. For video-accurate comments, use AI Video Analysis.</p>
        </div>
      )}
      ...
      {simChats.length>0&&(
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <button onClick={handleFallbackGenerate} className="text-xs text-white/30 hover:text-white/60 transition-colors">↺ Regenerate</button>
          </div>
          <div className="space-y-1 max-h-64 overflow-y-auto pr-1">
            {simChats.slice(0,50).map(m=>(
              <div key={m.id} className="flex items-center gap-3 p-2 bg-white/3 border border-white/5 rounded-lg hover:border-white/10 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <span className="text-[10px] text-white/25 mx-1">·</span>
                ...
                <span className="text-xs text-white/40 truncate max-w-[130px]">{m.message}</span>
              </div>
            ))}
...
function SettingsTab({slug,videoSource,onVideoSourceChange,simChats,onSimChatsChange,videoDuration,onVideoDuration}:{slug:string;videoSource:VideoSource|null;onVideoSourceChange:(s:VideoSource|null)=>void;simChats:SimChat[];onSimChatsChange:(c:SimChat[])=>void;videoDuration:number;onVideoDuration:(d:number)=>void}){
  ...
  const handleManualDur=(val:string)=>{
    ...
    if(mins>0){const d=mins*60;onVideoDuration(d);saveLS(`wf-duration-${slug}`,d);
      if(simChats.length>0){const r=rescaleSimChats(simChats,d);onSimChatsChange(r);saveLS(`wf-chats-${slug}`,r);}}
  };
    ...
    <div className="flex-1 flex overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {activeSection==="chatsim"&&(
          <ChatSimulatorSection slug={slug} simChats={simChats} onSimChatsChange={onSimChatsChange} videoDuration={videoDuration} onVideoDuration={onVideoDuration}/>
        )}
