      ...
      <div className="relative rounded-2xl overflow-hidden border-2 border-yellow-400/70 bg-[#0a0a0a]">
        {embedSrc ? (
          ...
          <div className="aspect-video flex flex-col items-center justify-center bg-gradient-to-br from-gray-900 via-black to-gray-900 gap-4">
            ...
            {/* Admin: paste video URL to embed */}
            {showInput && (
...
// ─── CTA Button — handles auth + checkout ─────────────────────────────────────
function CTAButton({ size = "lg" }: { size?: "lg" | "md" }) {
  ...
  const baseClass =
    "relative w-full max-w-xl mx-auto flex items-center justify-center gap-3 font-black rounded-2xl transition-all duration-200 disabled:opacity-70 disabled:cursor-not-allowed overflow-hidden"
  const sizeClass =
    size === "lg"
      ? "py-6 px-8 text-xl md:text-2xl"
      : "py-4 px-6 text-lg"
  ...
  return (
    <div className="flex flex-col items-center gap-3 w-full">
      <button
        onClick={handleClick}
        disabled={loading}
        className={`${baseClass} ${sizeClass} bg-yellow-400 hover:bg-yellow-300 text-black group`}
...
// ─── Spot counter (cosmetic) ──────────────────────────────────────────────────
function SpotCounter() {
  const [spots] = useState(387)
  return (
    <div className="flex items-center justify-center gap-2 text-sm text-gray-400">
      ...
      <span><span className="text-yellow-400 font-bold">{spots}</span> spots claimed · <span className="text-red-400 font-semibold">{500 - spots} remaining</span></span>
    </div>
  )
}
...
function FAQ({ q, a }: { q: string; a: string }) {
  ...
  return (
    ...
    >
      ...
      {open && <p className="text-gray-400 text-sm leading-relaxed mt-3 border-t border-white/10 pt-3">{a}</p>}
    </button>
  )
}
          ...
          <div className="relative mb-10">
            ...
            <div className="relative bg-yellow-400/8 border-2 border-yellow-400 rounded-3xl p-8">
              <p className="text-gray-400 text-base mb-1">
                Total Value: <span className="line-through">$3,285+</span>
              </p>
          ...
          {[
            {
              ...
              a: "Zero. If you can fill out a form, you can launch a webinar funnel with WebinarForge AI. We built it for non-tech people from the ground up.",
            },
            {
              q: "How fast can I launch my first webinar?",
              a: "Most users launch their first evergreen webinar funnel in under 10 minutes. The AI does 95% of the work — you just review and publish.",
            },
            {
              q: "Is this really a one-time payment?",
              a: "Yes. $49 one-time. No monthly fees, no hidden charges, no surprises. Early bird members lock in lifetime access permanently.",
            },
            {
              q: "What happens after the early bird closes?",
              a: "The price goes to $97/month — permanently. Once the 500 early bird spots are gone, this offer is gone forever. There's no way to get it back at this price.",
            },
            {
            ...
            },
            {
              ...
              a: "Most people who switch to WebinarForge AI never go back. It replaces 5–7 different tools and does everything automatically — often better.",
            },
          ].map(({ q, a }) => (
            <FAQ key={q} q={q} a={a} />
          ))}
      ...
      <section className="py-20 md:py-28 px-5 bg-yellow-400/5 border-t-4 border-yellow-400">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <div className="flex flex-wrap justify-center gap-5 text-sm text-gray-500 mb-8">
            <span className="flex items-center gap-1.5">🔒 256-bit SSL Secure</span>
            <span className="flex items-center gap-1.5">💳 All Cards Accepted</span>
